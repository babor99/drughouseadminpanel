...
import { CREATE_ADMIN_STUDENT, UPDATE_ADMIN_STUDENT } from '@/constants/constants'
...
const FormHeader = () => {
  ...
  function handleSaveAdminStudent() {
    setLoading(true)
    ...
    try {
      fetch(CREATE_ADMIN_STUDENT, {
        ...
        .then(data => {
          ...
          router.replace(getLocalizedUrl('/apps/store/students', locale))
        })
        .catch(error => {
          setLoading(false)
  ...
  function handleUpdateAdminStudent() {
    setLoading(true)
    ...
    try {
      fetch(`${UPDATE_ADMIN_STUDENT}${studentId}`, {
        method: 'PUT',
        credentials: 'include',
        headers: authHeaders.headers,
        body: formData,
  ...
  return (
    <div className='flex flex-wrap sm:items-center justify-between max-sm:flex-col gap-6'>
      ...
      <div className='flex flex-wrap max-sm:flex-col gap-4'>
        <Link href={getLocalizedUrl(`/en/apps/store/students`, locale)}>
        ...
        </Link>
        {
          studentId === 'new' ?
            <Button
              ...
              onClick={handleSaveAdminStudent}
            >
              Save
            </Button>
            :
            ...
            <Button
              variant='contained'
              disabled={!isValid || isEmpty(dirtyFields) || loading}
              onClick={handleUpdateAdminStudent}
            >
              Update
            </Button>
        }
