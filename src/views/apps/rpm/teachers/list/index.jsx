...
import { DELETE_RPM_TEACHER, GET_RPM_TEACHERS } from '@/constants/constants'
...
const TeacherList = () => {
    ...
    function getAllTeacher(seachParams) {
        ...
        try {
            fetch(`${GET_RPM_TEACHERS}?${objectToQueryString(seachParams)}`, authHeaders)
                .then(res => {
    ...
    const deleteTeacher = (rpmId) => {
        ...
        try {
            fetch(`${DELETE_RPM_TEACHER}${rpmId}`, authHeaders)
                .then(res => {
                    if (res.ok && [200, 201].includes(res.status)) {
                        return res.json()
                    }
