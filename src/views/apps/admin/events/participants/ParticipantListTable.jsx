...
const ListTable = (props) => {
  ...
  return (
    <>
      <Card>
        <CardHeader title={`Participant List (${currentEvent?.name})`} />
        {/* header buttons */}
                        ...
                        .map(n => {
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                {
                                  n?.event_enrollment_answers?.map((fieldValue, index) => {
                                    if (fieldValue?.file) return <div key={index} className='flex items-center'>
                                      <span >{fieldValue?.file.split('?')?.[0]?.split('EventCustomFieldValueFiles/')?.[1]}</span>
                                      <CustomIconButton
                                        className='ms-3 p-1'
                                        variant='contained'
                                        color='primary'
