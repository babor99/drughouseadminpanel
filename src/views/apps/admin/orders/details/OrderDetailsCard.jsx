  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('productName', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-3'>
            <img src={`${BASE_URL}/${row?.original?.product?.thumbnail}`} alt='' height={34} className='rounded' />
      ...
      columnHelper.accessor('price', {
        header: 'Price',
        cell: ({ row }) => <Typography>{`${row?.original?.unit_price}`}</Typography>
      }),
...
const OrderDetailsCard = ({ orderData, orderItems }) => {
  return (
    <Card>
      <CardHeader
        title='Order Details'
